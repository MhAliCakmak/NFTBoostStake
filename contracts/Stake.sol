...
contract Staking is ReentrancyGuard {
    ...
    uint256 public constant REWARD_RATE = 2000000000000000000;
    uint256 public s_lastUpdateTime;
    uint256 public s_rewardPerTokenStored;

    uint256[] public nftValues = [5, 10, 15, 20];
    uint256[] public timeValues = [0, 0];
    ...
    function fetchStakeListed() public view returns (address[] memory) {
        address[] memory stakers = new address[](s_totalStaker);
        uint256 counter = 0;
        for (uint256 i = 0; i < s_totalSupply; i++) {
            if (s_balances[stakers[i]] > 0) {
                stakers[counter] = stakers[i];
                counter++;
            }
        }
        return stakers;
